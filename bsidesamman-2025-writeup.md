...
<ul>
<li><a href="#targeted-kerberoasting">Targeted Kerberoasting</a></li>
...
The CTF ran across 2 days, where the scenario was slightly modified from the original one. This was done in order to maintain the integrity of the competition.
...
#### Day 2 - Unauthenticated Access
  
In this version the initial access is done by password spraying, the users are given the necessary IPs and the user given a hint if no progress has been made in 5 minutes. The hint provided was:
* "Everything you need is provided to you, the username and the IPs. Go back to the basics of Active Directory attacks and enumerations".

The first step of the updated scenario is to authenticate to the domain controller using the username as the password. This is done using the `nxc` tool:
