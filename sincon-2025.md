...
This post will provide an overview of the more "challenging" part of the CTF, which involved bypassing [Just Enough Administration (JEA)](https://learn.microsoft.com/en-us/powershell/scripting/security/remoting/jea/overview?view=powershell-7.5) and exploiting [ESC8](https://www.thehacker.recipes/ad/movement/adcs/unsigned-endpoints#web-endpoint-esc8) from a pivot in a restrictive environment by temporarily unbinding SMB ports without the use of [PortBender](https://github.com/praetorian-inc/PortBender).
...
Additionally, the configuration required to enable ESC8 is vulnerable by default when installing `Web Enrollment` on the CA server. The following commands were run on a default installation of ADCS to enable the vulnerability:
